...
You can override the error id by using the `options` argument with an `id` field set.
Only strings are accepted, otherwise default uuid.v4 is used.

This is useful if you want to keep track of the id somewhere else in the application,
for example sending it back to the client together with the error response.
